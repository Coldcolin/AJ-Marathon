...
const routes = [
    ...
    { path: '/get_involve', name: 'Get involved' },
    { path: '/blog', name: 'News & Updates' },
                ...
                <ul className='header_menu_wrapper'>
                    {
                        routes.map((route) => (
                            <li key={route.path}>
                                <NavLink to={route.path} className={({isActive})=>(isActive ? "nav_active" : 'menu_not_active')} >{route.name}</NavLink>
                            ...
                            <ul className='header_mobile_menu_Ul'>
                                {
                                    routes.map((route) => (
                                        <li key={route.path}>
                                            <NavLink to={route.path} className={({isActive})=>(isActive ? "nav_active" : 'menu_not_active')} onClick={() => {
                                                setMenu(false)
                                            }}>{route.name}</NavLink>
                                        </li>
                                    ))
